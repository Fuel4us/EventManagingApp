...
public class FunctionCall implements Expression {
     ...
     * @throws IllegalFunctionCallException if the arguments passed to the
     * function did not match its parameters
     */
    public FunctionCall(Function function, Expression... args)
        ...
        for (int i = 0; i < args.length; i++) {
            if (params.length <= i && !function.isVarArg()) // Too many arguments
            {
                throw new IllegalFunctionCallException(function, null, args[i]);
            }
        }
        for (int i = params.length - 1; i >= 0; i--) {
            if (i >= args.length && !params[i].isOptional()) // Too few arguments
            {
